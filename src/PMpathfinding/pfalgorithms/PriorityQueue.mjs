...
export default class PriorityQueue
{
	...
	{
        console.log("reorderUp|Length:", orderedArr.length, distancesMap.get(orderedArr[orderedArr.length -1]), "stoca");   
        for (let idx = orderedArr.length - 1; /*idx > 0 && */distancesMap.get(orderedArr[idx]) < distancesMap.get(orderedArr[idx - 1]); idx--)
		{
            console.log("reorderUp Managing INDEXES:", idx, idx - 1);//distancesMap.get(orderedArr[idx]), distancesMap.get(orderedArr[idx - 1]))
	...
	// {
	// 	for (let a = 0, len = orderedArr.length - 1; a < len; a++)
	// 	{
